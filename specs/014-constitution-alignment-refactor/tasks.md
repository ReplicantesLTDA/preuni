...
- [X] T016 [US4] Created `.github/workflows/backend-ci.yml`: applies `infra/migrations/*` via `psql` against a live Postgres service (golang-migrate's up/down naming doesn't match this repo's plain-numbered `.sql` files — pre-existing, out of scope), then `bash backend/scripts/check-coverage.sh` (real gate, provisional 30% floor — see T060). `golangci-lint`/`gofmt` run **non-blocking**: the existing codebase has repo-wide gofmt/import-order drift from a toolchain-version difference, predating this pipeline — blocking on it would fail every PR for reasons outside its scope. `go build`/`go vet` verified locally.
- [X] T017 [P] [US4] Created `.github/workflows/correction-service-ci.yml`: `ruff check` + `ruff format --check` (both genuinely clean, repo-wide — verified locally), `import-linter` (verified locally: 2/2 contracts kept), `pytest tests/unit --cov=src --cov-fail-under=45` (real, verified locally: 92 passed, 50.95% ≥ 45%), `pytest tests/integration -m integration` + golden (fake provider) — **not verified locally** (no live Postgres in this environment; will get their first real run in CI). `mypy` runs **non-blocking**: 25 pre-existing errors predate this refactor (confirmed via diff — none in files this refactor touches beyond pre-existing patterns).
