...
- [ ] T012 Implement internal service-to-service auth (shared bearer token, config-driven) for Go → correction-service calls: emit/check in `backend/app/internal/adapters/correctionclient/` and `corretor-redacao/src/api/internal_auth.py`
- [ ] T013 Remove auth/users/quota endpoints and route registrations from `corretor-redacao/src/api/` (drop `/auth/*` and quota-checking code paths per research.md §2)
- [ ] T014 [P] Update the correction worker in `corretor-redacao/src/workers/` to claim rows from `correction_jobs` (via `SELECT ... FOR UPDATE SKIP LOCKED`) instead of `corrections` directly, writing results into `corrections` keyed by the job id
- [ ] T015 Wire new domain routers (`essay`, `streak`, `social`, `gamification`) into `backend/app/internal/router/router.go`
...
- [ ] T016 [US4] Create `.github/workflows/backend-ci.yml`: `go test -p 1 -coverprofile=coverage.out ./...`, fail if `go tool cover -func` total < 90%
- [ ] T017 [P] [US4] Create `.github/workflows/correction-service-ci.yml`: `ruff check`, `mypy`, `import-linter`, `pytest --cov=src --cov-fail-under=90` (reusing `corretor-redacao`'s existing `make lint`/`make test-*` targets)
- [ ] T018 [US4] Extend `.github/workflows/mobile-ci.yml` to fail the job when Jest coverage is below 90%
- [ ] T019 [US4] Fill in `.pre-commit-config.yaml` (from T003) with `gofmt`/`golangci-lint`, `ruff`/`mypy`, `eslint`/`tsc` hooks scoped to changed files per codebase
- [ ] T020 [US4] Add `mobile/jest.config.js` (or `package.json` `jest.coverageThreshold`) enforcing 90% global coverage
- [ ] T021 [US4] Add `--cov-fail-under=90` to `corretor-redacao/pyproject.toml` pytest/coverage config
- [ ] T022 [US4] Write `infra/branch-protection.md` documenting the required GitHub branch-protection settings for `main` (3 required status checks + 1 required human review, no force-push) and a `gh api` script to apply them
- [ ] T023 [US4] Run quickstart.md's "Run the full test/coverage suite locally" commands against a clean checkout and fix any command that doesn't match the actual Makefile/package.json scripts
