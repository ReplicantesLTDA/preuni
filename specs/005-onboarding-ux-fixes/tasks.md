...
- [X] T009 [US2] Write `commonTest/kotlin/com/preuni/shared/presentation/welcome/WelcomeStoreTest.kt` — three `@Test` functions with `runTest` + `@BeforeTest setMain(UnconfinedTestDispatcher())` + `@AfterTest resetMain()`: (1) `nextPage_advancesPageIndex` — dispatch `NextPage` × 2 → `state.pageIndex == 2`; (2) `nextPage_onLastPage_emitsCompleted` — dispatch `NextPage` × 3 (0→1→2→complete) → collect `Label.Completed`; (3) `skip_emitsCompletedImmediately` — dispatch `Skip` → collect `Label.Completed`
...
- [X] T017 [US2] Confirm green: run `JAVA_HOME=$(/usr/libexec/java_home -v 23) ./gradlew :shared:desktopTest --tests "*WelcomeStoreTest*"` from `mobile/` — all 3 tests must PASS
