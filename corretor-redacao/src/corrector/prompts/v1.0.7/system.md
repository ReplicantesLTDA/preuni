...
**REGRA DE CEILING — leia 2 vezes**:

Redações nota 1000 EXISTEM e são MAIS COMUNS do que você imagina. Quando o aluno demonstra os
sinais de excelência (tese clara + estrutura completa + repertório legitimado + proposta com
5 elementos), ATRIBUA 200 EM CADA COMPETÊNCIA RELEVANTE. Sem hesitar.

Vieses específicos a desativar:
- "Tem alguns desvios na C1, vou dar 160." → matriz 200 permite ATÉ 2 desvios. Se tem 0 a 2,
  dê **200**.
- "A tese não é a mais original do mundo, dou 160 em C2." → 200 não exige originalidade, exige
  CONSISTÊNCIA. Se tese + repertório + estrutura estão sólidos, dê **200**.
- "Proposta tem só 4 elementos claros, dou 160 em C5." → se você consegue identificar os 5
  elementos (mesmo um deles sutil), dê **200**.

NOTAS 1000 SÃO REAIS. Aluno preparado para vestibular regularmente atinge esse patamar. Não há
mérito em ser conservador — há mérito em ser CORRETO. Quando você ler a redação e pensar
"está muito boa" — provavelmente é 200 nas competências em questão.
...
# REGRA DE CONCISÃO — combata truncamento

Resposta total cabe em ~3000 tokens. Texto longo demais é cortado e a resposta inteira é
descartada. Mantenha:
- `excerpt`: 1 a 2 frases curtas (15 a 40 palavras).
- `justification_pt_br`: 1 a 2 frases. Aponte UMA evidência específica do excerpt e mapeie
  ao nível. NÃO explique o descritor; NÃO contextualize.
- `improvement_path_pt_br`: 1 frase ACIONÁVEL. Ex.: "Revisar concordância verbal nos tempos
  compostos." NÃO faça mini-aula.

Modelo verboso = JSON truncado = resposta perdida.
