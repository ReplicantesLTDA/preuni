// Package testhelper provides shared utilities for user handler integration tests.
...
// TestSigningKey is the HMAC key used to sign test JWTs.
const TestSigningKey = "test-signing-key-32-bytes-minimum!"

// SetupTestDB opens a pgxpool connection to the database at TEST_DB_URL,
// runs all SQL migrations from infra/migrations/user/ (in filename order),
// and registers a cleanup function that truncates every users table after
...
// MakeTestJWT generates a signed HS256 JWT for the given userID, valid for 1 hour.
// The token is signed with TestSigningKey so tests can use pkgmw.RequireAuth with
// the same key.
func MakeTestJWT(t *testing.T, userID string) string {
	...
	if err != nil {
		t.Fatalf("testhelper: sign test JWT: %v", err)
	}
	return token
}
...
func runMigrations(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	...
	for _, path := range sqlFiles {
		...
		if _, err = pool.Exec(ctx, string(sql)); err != nil {
			// Migrations may already exist — log and continue.
			t.Logf("testhelper: migration %q (may be already applied): %v", filepath.Base(path), err)
		}
	}
}

// cleanupTables truncates all users tables so each test starts with a clean slate.
